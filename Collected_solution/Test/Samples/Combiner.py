...
for angle in angles:
    filename = f"60dB_gain/angles_degree_{angle}.npz"
    ...
    if os.path.exists(filename):
        ...
        with np.load(filename) as data:
            ...
            combined_data[f"angle_{angle}"] = data[key]
    else:
        print(f"Warning: {filename} not found.")

# Save everything into a single consolidated .npz file
np.savez("combined_angles.npz", **combined_data)
print("Files successfully combined into 'combined_angles.npz'!")
