...
title("Average Received Packets Indoors (60s)")

% Bottom Plot: Lost Packets
ax2 = subplot(2, 1, 2); hold on; grid on;
ylabel("Packets Lost [-]")
xlabel("Time [s]");
title("Average Lost Packets Indoors (60s)")
...
for g = gains
    ...
    for i = 0:numTrials-1
        % Construct filenames
        dataFile = sprintf('results_grp%dg%d.csv', g, i);
        lostFile = sprintf('lost_results_grp%dg%d.csv', g, i);
