...
class SPPDecoder:
    ...
    def decode(self, bits: list) -> Optional[dict]:
        ...
        sync_index = self.find_sync_word()
        if sync_index == -1:
            return None
        
        # Remove bits before sync word
        self.buffer = self.buffer[sync_index + 32:]
        
        # Parse primary header
        if len(self.buffer) < (self.PRIMARY_HEADER_SIZE * 8):
            return None
        
        header_bits = self.buffer[:self.PRIMARY_HEADER_SIZE * 8]
    ...
    def find_sync_word(self) -> int:
        """Find sync word position in buffer"""
        sync_bits = bin(self.sync_word)[2:].zfill(32)
        sync_str = ''.join(map(str, self.buffer[:32])) if len(self.buffer) >= 32 else ""
        
        for i in range(len(self.buffer) - 31):
            candidate = ''.join(map(str, self.buffer[i:i+32]))
            if candidate == sync_bits:
                return i
        return -1
